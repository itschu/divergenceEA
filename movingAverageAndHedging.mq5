...
void checkTrend(double lotSize){
   double Ask=NormalizeDouble(SymbolInfoDouble(_Symbol, SYMBOL_ASK), _Digits);
   MqlRates PriceInfo[];
   ...
   int mA = iMA(_Symbol, PERIOD_CURRENT, 50, 0, MODE_SMA, PRICE_CLOSE);
   CopyBuffer(mA, 0, 0, 2, mAArray);
   double diff =   PriceInfo[1].high - mAArray[1];
   if(PriceInfo[1].high > mAArray[1] && diff > 0.0008 ){
